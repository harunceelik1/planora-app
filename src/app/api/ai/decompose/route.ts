...
export async function POST(req: Request) {
  ...
  const { title, projectId, sprintId, locale } = body;

  if (!title || !projectId) {
    return NextResponse.json(
      { error: "Missing title or projectId" },
      { status: 400 }
    );
  }
...
[
  ...
  if (err?.status === 404) {
    console.error("Model bulunamadı:", err.message);
    subtasks = fallbackDecompose(title);
    usedFallback = true;
    break;
  }
  ...
  try {
    const [lastIssue, lastOrderIssue] = await Promise.all([
      db.issue.findFirst({
        where: { projectId },
        orderBy: { number: "desc" },
      }),
    ...
    return NextResponse.json(
      { error: "Could not create subtasks in database" },
      { status: 500 }
    );
  }
}