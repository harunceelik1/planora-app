...
export function DataTable<TData, TValue>({
  ...
  if (data.length === 0) {
    return (
      <div className="flex flex-col items-center justify-center py-12 text-center border-t border-dashed border-slate-200 mt-4 animate-in fade-in duration-500">
        <div className="bg-slate-50 p-4 rounded-full mb-3">
          <ClipboardList className="h-8 w-8 text-slate-300" />
        </div>
        <h3 className="text-slate-900 font-medium text-sm">No tasks found</h3>
        ...
        <TableBody>
          {table.getRowModel().rows.map((row) => (
            <TableRow
              ...
              className="bg-white hover:bg-slate-50 shadow-sm transition-all border-none group"
            >
              {row.getVisibleCells().map((cell, index) => {
                ...
                return (
                  <TableCell
                    key={cell.id}
                    className={cn(
                      "py-4 border-y border-slate-100",
