...
import { IssueLabelBadge, normalizeIssueLabels } from "../issue/issue-labels";
                ...
                <div className="space-y-1.5">
                  ...
                  >
                    <SelectTrigger className="h-8 border-dashed bg-transparent hover:bg-slate-50 dark:hover:bg-slate-900 transition-colors w-full justify-start gap-2 shadow-none dark:text-slate-100">
                      ...
                      <SelectValue
                        placeholder={t("placeholders.estimate")}
                      />
                    </SelectTrigger>
                    <SelectContent>
                      ...
                      <div key={a.id} className="flex gap-3">
                        ...
                        <div className="flex flex-col flex-1">
                          ...
                          <p className="text-sm text-slate-600 dark:text-slate-300 mt-0.5">
                            {(() => {
                              const userName = a.user?.name || t("unknownUser");
                              switch (a.type) {
                                case "ASSIGNEE_CHANGED": {
                                  const from = a.data?.from ? a.data.from : "—";
                                  const to = a.data?.to ? a.data.to : "—";
                                  return `${userName} atamayı ${from} → ${to} yaptı`;
                                }
                                case "COMMENT_ADDED":
                                  return `${userName} yorum ekledi: "${(a.data?.content || "").slice(0, 140)}"`;
                                case "ISSUE_UPDATED":
